...
export async function parseResumeWithGemini(text: string, apiKey: string): Promise<ExtractedProfile> {
  const prompt = `
    ...
    - graduationDate (string: Expected or actual graduation date in YYYY-MM format. If month is unknown, use 01)
    - education (array of objects):
      - institutionName (string)
      - degree (string)
      - fieldOfStudy (string)
      - startDate (string: YYYY-MM-DD format or YYYY-MM. Use valid months 01-12 and days 01-31. If unknown, use 01)
      - endDate (string: YYYY-MM-DD format, YYYY-MM, or null if current. Use valid months 01-12 and days 01-31. If unknown, use 01)
      - isCurrent (boolean)
      - description (string)
